...
const getCat = async (catId: number): Promise<Cat> => {
  const sql = promisePool.format(
    'SELECT cat_id, cat_name, weight, owner, filename, birthdate, coords, sssf_user.user_name AS ownername FROM sssf_cat JOIN sssf_user ON sssf_user.user_id = sssf_cat.owner WHERE cat_id = ?;',
    [catId]
  );
  console.log(sql);
